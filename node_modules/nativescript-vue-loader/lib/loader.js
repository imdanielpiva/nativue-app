...
module.exports = function (content) {
  ...
  const isNative = this.target === 'nativescript'
  ...
  // <script>
  output += '/* script */\n'
  const script = parts.scripts.find(script => script.attrs.hasOwnProperty(isNative ? 'native' : 'web')) || parts.script
  if (script) {
    if (options.esModule) {
      output += script.src
        ? (
  ...
  // <template>
  output += '/* template */\n'
  const template = parts.templates.find(template => template.attrs.hasOwnProperty(isNative ? 'native' : 'web')) || parts.template
  if (template) {
    if (options.esModule) {
      output +=
        (template.src
